  ...
  test("{comp_name} universal bit-exact circuit verification under Verilator") {{
    ...
    val compiled = SimConfig.withVerilator.withConfig(spinalConfig).compile {{
      ...
      val inputValues = (0 until inElems).map {{ idx =>
        HWArithmetic.fromDouble((((idx % 5) + 1) * 0.125).toFloat, 8, 7)
      }}
      val inFloats = inputValues.map(f => HWArithmetic.decode(f, 8, 7).toFloat)
      ...
      val replicaResult = ModelReplica.forwardWithTrace(layers, inputShape, inputValues, packed)
