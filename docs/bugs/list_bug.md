...
- **Statut** : corrigé (Wave 4 commit 9) — `DivOp` refuse tout type non-`FloatML` à l'élaboration (`require` explicite, message orientant vers la Wave 5). Preuve rouge/verte, fallout tests/formels et backlog Q-format : voir OPS-03 (même commit). **Wave 5 step 5A** : la sémantique est désormais définie par **ONNX `Div`** (troncature vers zéro exacte, I/O même dtype, saturation div0/`INT_MIN/-1`, jamais de wrap) ; `DivOp` implémente `IntDiv` (restoring divider déroulé magnitudes+signe) pour SInt/UInt ≤8 bits, I16/I32 restant refusés en attendant le divider itératif (step B). Rouges avant : `DivTest` I8/U8 (ex-`require`), verrouillés par des sims exactes ; golden Python `test_div.py::i8_div_bits` (ONNX trunc) ; non-régression `SoftmaxTest|AttentionTest|RequantizeTest` 6/6 ✅, `DivFormal` ✅. Q15/`fracBits` abandonnés : ONNX ne définit pas de division quantifiée et TFLite n'a pas d'op int8 runtime standard.
