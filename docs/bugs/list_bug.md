...
- **Statut** : corrigé (Wave 4 commit 9) — refus explicite à l'élaboration (`require` avec message clair) sur `DivOp`/`SigmoidOp`/`TanhOp` pour tout type non-`FloatML` ; la sémantique Qm.n (division + sigmoïde quantifiée, scales TFLite) devient un chantier Wave 5 tracé dans `docs/full_roadmap.md` §3 et `docs/rounding_policy.md` §7. **Wave 5 step 5A** : volet `DivOp` résolu via la sémantique ONNX exacte ≤8 bits (voir OPS-12), étendu >8 bits au step 5B. **Wave 5 step 5C** : volet activations résolu — `SigmoidOp`/`TanhOp` acceptent I8/U8 quantifiés avec la convention TFLite int8 (LUT 256 entrées pleine échelle via `QuantActivation`, LOGISTIC out 1/256 zp -128 (int8) / 0 (uint8), TANH out 1/128 zp 0/128 ; scale/zp d'entrée par `LayerSpec` et `Sequential`) ; I16 refusé (ROM 64K entrées) ; réplica `sigmoidInt`/`tanhInt` + handlers + `ModelReplica` alignés. Rouges : tests I8/U8 (ex-`require` OPS-03) ; vérifs : Scala I8/U8/FP8/BF16 + scales/zp ✅, formels `Sigmoid/TanhFormal` i8+FP4+FP9 ✅, pytest `test_sigmoid`/`test_tanh` sweeps 256 codes I8 ✅, suite universelle 10/10 ✅, `SequentialTest|AcceleratorTest|SoftmaxTest|Sequential2DTest` ✅. `ReciprocalOp` int (donc `Softmax1D` int, compilation seule non vérifiée) reste sans sémantique Qm.n : chantier dédié, hors step 5C. Rouges avant : `DivTest`/`SigmoidTest`/`TanhTest` « integer types are rejected at elaboration » (`Expected exception ... but no exception was thrown`). Fallout re-baseliné : tests I8/I16 convertis en tests de rejet, goldens Python int retirés (`test_div.py`/`test_sigmoid.py`/`test_tanh.py`), formels `DivFormal_I8`/`SigmoidFormal_I8`/`_I10`/`TanhFormal_I8`/`_I10` retirés (variantes FP4/FP9 conservées), `Sequential2DTest` activations int déplacées en BF16 via `Cast`. Non-régression : suites Scalas 4/4 ✅ (12 tests), pytest div/sigmoid/tanh FP 6/6 ✅, formels Div/Sigmoid/Tanh FP PASS ✅, suite universelle 10/10 en RNE ✅ et lane trunc revalidée ✅ (le replica oracle, resté en troncature, a été re-baseliné RNE/switch-aware dans le commit transverse : `UniversalMixed2DDemo` et `UniversalResidualDemo` étaient rouges à cause de lui, pas du RTL).
...
- **Statut** : corrigé (Wave 4 commit 9) — `DivOp` refuse tout type non-`FloatML` à l'élaboration (`require` explicite, message orientant vers la Wave 5). Preuve rouge/verte, fallout tests/formels et backlog Q-format : voir OPS-03 (même commit). **Wave 5 steps 5A+5B** : la sémantique est désormais définie par **ONNX `Div`** (troncature vers zéro exacte, I/O même dtype, saturation div0/`INT_MIN/-1`, jamais de wrap) ; `DivOp` implémente `IntDiv` (restoring divider déroulé magnitudes+signe) pour SInt/UInt ≤8 bits et un divider restoring **série** pour >8 bits (I16/I32, 1 beat en vol, latence fixe `width + 2`, bit-identique au combinatoire). Rouges avant : `DivTest` I8/U8 (ex-`require` step 5A) puis I16/I32 (ex-`require` « unimplemented above 8 bits », step 5B), verrouillés par des sims exactes ; golden Python `test_div.py::int_div_bits` (ONNX trunc, I8+I16) ; non-régression `SoftmaxTest|AttentionTest|RequantizeTest` 6/6 ✅, `DivFormal` ✅. Q15/`fracBits` abandonnés : ONNX ne définit pas de division quantifiée et TFLite n'a pas d'op int8 runtime standard.
...
- **Statut** : corrigé — `getWeightShape()/getBiasShape()` retournent `Seq(features)` (1D) pour les deux specs, en cohérence avec les ports `gamma`/`beta` `Tensor(..., Seq(channels))`. Consommateurs vérifiés : `Sequential.scala:319-320` et `WeightMemoryLayout.scala:102-103` utilisent `head`/`product`, donc layout octets et lanes inchangés (aucun impact fonctionnel). Test rouge : `LayerSpecTest` « weight and bias shapes are 1D (LAY-02) » (`List(4,1) did not equal List(4)` avant). Non-régression : `LayerSpecTest` ✅, suites Scala BatchNorm/LayerNorm ✅, `SequentialTest` ✅, pytest batchnorm+layernorm 8/8 ✅, `test-all-python -k accelerator` ✅. Garde générique ajouté ensuite : `LayerSpecTest` « LayerSpec metadata matches hardware ports (LAY-02) » (`LayerSpecConformanceComp` instancie un composant par famille — Linear/Conv1D/Conv2D/BatchNorm1D/LayerNorm1D/ClassicalAttention — et compare `io.w`/`io.b`/`gamma`/`beta`/`io.y` aux `getWeightShape()/getBiasShape()/getOutShape()` ; attention : somme des 4 ports `[embedDim, embedDim]` = `[4·embedDim, embedDim]`, aucun biais) et « weightless LayerSpecs declare no weight/bias region (LAY-02) » (aurait attrapé la régression `Seq(0)`→non-zéro du trigger DMA `Sequential.scala:204`). Sensibilité prouvée en rouge par injection temporaire de `Seq(4,1)` : `LAY-02 conformance: BatchNorm1D gamma hardware port List(4) != LayerSpec List(4, 1)`. Non-régression : `test-all -k "LayerSpecTest|SequentialTest|AcceleratorTest"` 4/4 ✅.
...
- **Limitation résiduelle distincte — corrigée (Wave 5 step 4)** : en FP8, si `diff²` sous-flue alors que `diff≠0`, `var` tombe à 0 et `invStd` saturait (E4M3 : 448, d'où `norm = 0.0625×448 = 28.0` sur la trame `[0.5, 0.5625, 0.5, 0.5]`, `0.0625² = 2^-8` flushé par FTZ). Politique d'epsilon retenue : `floatEncodeFn(1e-5)` si représentable (**BF16 inchangé**), sinon **plus petit normal positif** `(exp=1, mant=0)` — E4M3 `2^-6`, E2M1 `1.0` (FTZ ne laisse aucun sous-normal ; décision utilisateur : fallback conditionnel uniforme, pas de `require` FP4). Rouge avant : `LayerNormTest` « small-variance FP8 frame keeps invStd finite (LAY-04 residual) » (`(exp=11, mant=6)` = 28.0 au lieu de `(exp=6, mant=0)` = 0.5) ; vert après. Réplica `LayerReplicas.layerNorm1D` (même politique) et golden `ops.py::layernorm_hw` (ajout `floatml_add(var, eps)` avant `rsqrt`) alignés. Non-régression : `LayerNormTest` ✅ (I8/FP8/BF16 + constant frames), suite universelle 10/10 ✅ (`UniversalPoolNormDemo` LayerNorm FP8 bit-exact via le réplica), `test_layernorm1d.py` ✅.
