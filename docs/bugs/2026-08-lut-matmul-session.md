...
## M4 — bug découvert au passage (PRÉEXISTANT, à corriger)
...
**Cause probable (à confirmer)** : une géométrie de battements « 1 beat = 1
...
alors que le Linear 4-lanes émet **72 battements par rangée-neurone**. Les swaps
ping/pong du `StreamDoubleBuffer` comptent bien en **éléments** (`pingFull`
/pongFull) — le bug n'est pas là ; il est dans la couche de contrôle.

**Bisect réalisée (suites chaînées → knob `MNIST_WLANES`)** :
...
Toutes les phases **sauf RELOAD** (baseline/rise/resident×N) sont déjà bit-exact
à 4 lanes. La corruption ne touche que le **2e refetch DDR** sous
résidence/prefetch, dès que `chunksK ≥ 4` (streams poids longues). L'hypothèse
dominante : course de synchronisation dans le monde « reArm + residentHold »
exposée par la durée du flux (à 288/96 l'ancienne géométrie, la course était
trop étroite pour se voir) — **pas encore identifiée** : prochaine étape = probe
nu-op isolant DMA+StreamDoubleBuffer+Streamer avec reArm et lanes 96/4.

**Corrections accessoires (faites)** : les suites `Chained`/`Continuous`/
`Banded` appelaient `Mnistw4a8Replica.logits(img)` (repli 288-lanes) → passe en
`logitsK(img, w4Lanes)` (repli du modèle).

**Prochaine étape** : A/B `MNIST_WLANES` sur les suites chaînées, probe
op-nu (style K1ConvCountTest) du préchargement à 4 lanes, fix ciblé, puis
re-lance des 4 suites + 20/20.

---

*Session : août 2026 — commits associés `4fea0c3` (M2 lanes) + `0bab5f7` (M3 temporal).*
