...
- [ ] **Tiling & Resource Scaling (tile/slice model, future work)**: first-class M/K/N **slices** (data blocking -> BRAM/DDR traffic) and M/N **tiles** (instantiated parallelism -> LUT/DSP), plus `lanes` for the datapath width, so ops are shaped to the board budget instead of their shape. Global `TilingPolicy` + per-layer resolved values + board-driven planner with fail-fast fit checks; prerequisite for the Phase-3 folded core. Vocabulary, constraints and phases R0-R3: `docs/tiling_resource_scaling.md`.
...
- [x] **Accelerator SoC Python/Cocotb Co-Simulation (Sep 2026)**: [`tests/python/test_accelerator.py`](../tests/python/test_accelerator.py) drives two generated `Accelerator` tops through the full Mill → Verilog → cocotb path, with Python playing both the AXI4-Lite master and the AXI4 slave: (1) identity-passthrough one-shot — START, TILE_CNT, busy/done; (2) continuous RUN/STOP auto-advance over 3 pre-loaded frames, bit-exact and silent after STOP; (3) DMAWriter write-back (CSR 0x20/0x24/0x28) verified word-by-word with `outStream` silent; (4) runtime dequantization boundary `Cast(BF16, runtimeScale = true)` programmed through CSR 0x30 (default 1.0 readback, scales 0.5/2.0/−1.0 bit-exact) — first dynamic validation of the CSR wiring used by [`inference.py`](../examples/Mnist/inference.py).
...
- [ ] **Weight Residency / Prefetch End-to-End Regression (test debt)**: the only end-to-end suites covering CSR `0x10` (WEIGHT_RESIDENT/PREFETCH_EN) and `0x14` (RELOAD) — `WeightResidentChainTest`, `WeightPrefetchChainTest` (and `MnistChainedTest`, `MnistContinuousTest`, `BandTilingTest`, `SdbSwapTb`) — live in `archive/test/examples/` and are NOT compiled/run by `test-all`; `open-mysteries.md`, `full_roadmap.md` §8 and `test-budget.md` still cite them as active. Consequence exposed by BUG-NN-01: the eager-fetch branch of `Sequential` (`prefetchWorldX`: reloadPending × loadCanAccept, no `ready` in `valid`) has no end-to-end check in the live suite (only the buffer-level `StreamDoubleBufferPrefetchFormal`). Action: revive the chain tests into `spinalML/test/src/spinalML/examples/` (or port them to `tests/python/test_accelerator.py` with a weight-bearing top + 0x10=0b11/0x14 writes), and fix the stale doc references.
