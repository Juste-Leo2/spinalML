...
import spinalML.layers.BatchNorm1D
...
case class BatchNormTestComp[T <: Data](dataType: HardType[T]) extends Component {
  ...
  val norm = BatchNorm1D(dataType, channels, 1)
  norm.io.x <> io.x
...
class BatchNormFormal_I8 extends Component {
  val dut = FormalDut(BatchNormTestComp(SInt(8 bits)))
...
class BatchNormFormal_FP4 extends Component {
  val dut = FormalDut(BatchNormTestComp(FP4_E2M1()))
...
object BatchNormFormal {
  def main(args: Array[String]): Unit = {
    FormalConfig
      ...
      .doVerify(new BatchNormFormal_I8, "batchnorm_i8")
    ...
    FormalConfig
      ...
      .withEngies(List(SmtBmc(solver = SmtBmcSolver.cvc4)))
      .workspacePath("formal")
      .doVerify(new BatchNormFormal_FP4, "batchnorm_fp4")
  }
}
