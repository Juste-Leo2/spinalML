...
import spinalML.poolings.AvgPool1DOp
...
case class AvgPool1DTestComp[T <: Data](dataType: HardType[T]) extends Component {
  ...
  val pool = AvgPool1DOp(dataType, L = 4, channels = 2, poolSize = 2, stride = 2)
  pool.io.a <> io.a
  io.c <> pool.io.c
}

class AvgPool1DFormal_I8 extends Component {
  val dut = FormalDut(AvgPool1DTestComp(SInt(8 bits)))
...
class AvgPool1DFormal_FP4 extends Component {
  val dut = FormalDut(AvgPool1DTestComp(FP4_E2M1()))
...
object AvgPool1DFormal {
  def main(args: Array[String]): Unit = {
    FormalConfig
      ...
      .doVerify(new AvgPool1DFormal_I8, "avgpool1d_i8")
    ...
    FormalConfig
      ...
      .withEngies(List(SmtBmc(solver = SmtBmcSolver.cvc4)))
      .workspacePath("formal")
      .doVerify(new AvgPool1DFormal_FP4, "avgpool1d_fp4")
  }
}
