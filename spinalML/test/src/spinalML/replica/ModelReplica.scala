...
object ModelReplica {
  ...
  private def dispatchLayer(
    ...
    case ln: LayerNorm1D => DenseHandlers.evalLayerNorm1D(ln, curTensor, curShape, wInfo)
    ...
    case ap: AvgPool2D   => PoolHandlers.evalAvgPool2D(ap, curTensor, curShape)
    case ap: AvgPool1D   => PoolHandlers.evalAvgPool1D(ap, curTensor, curShape)
    ...
    case sm: Sigmoid     => ActivationHandlers.evalSigmoid(sm, curTensor, curShape)
    case th: Tanh        => ActivationHandlers.evalTanh(th, curTensor, curShape)
    ...
    case rq: Requantize  => TransformHandlers.evalRequantize(rq, curTensor, curShape)
