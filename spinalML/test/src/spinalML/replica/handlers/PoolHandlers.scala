...
import spinalML.nn.{AvgPool1D, AvgPool2D, MaxPool1D, MaxPool2D}
...
object PoolHandlers {
  ...
  def evalAvgPool2D(
    ...
    val nextTensor: ReplicaTensor = curTensor match {
      ...
      case ft: FloatTensor =>
        ...
        val pooled = LayerReplicas.avgPool2D(arr3D, p.poolSize, p.stride, ft.expBits, ft.mantBits)
        val flat = LayerReplicas.flatten(pooled)
        FloatTensor(nextShape, flat, ft.expBits, ft.mantBits)
    }
    (nextShape, nextTensor)
  ...
  def evalAvgPool1D(
    ...
    val nextTensor: ReplicaTensor = curTensor match {
      case it: IntTensor =>
        val arr2D = Array.ofDim[Long](l, c)
        var idx = 0
        val raw = it.asInts
        for (pos <- 0 until l; ch <- 0 until c) {
      ...
      case ft: FloatTensor =>
        ...
        val pooled = LayerReplicas.avgPool1D(arr2D, p.poolSize, p.stride, ft.expBits, ft.mantBits)
        val flat = ArrayBuffer[F]()
        for (pos <- pooled.indices; ch <- 0 until c) flat += pooled(pos)(ch)
