...
case class MaxPool1DTestComp() extends Component {
  val io = new Bundle {
...
class MaxPool1DTest extends AnyFunSuite {
  test("Test streaming MaxPool1D operation on I8 tensors") {
    SimConfig.withWave.compile(MaxPool1DTestComp()).doSim { dut =>
      ...
      // Send sequence: 5, 8, 3, 4
      val sequence = Array(5, 8, 3, 4)
      var i = 0
      ...
      assert(dut.io.c.stream.payload(0).toInt == 8) // max(5, 8)
