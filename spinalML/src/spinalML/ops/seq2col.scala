 ...
 */
case class Seq2ColOp[T <: Data](dataType: HardType[T], L: Int, C: Int, K: Int, outLanes: Int, inLanes: Int = 1) extends Component {
  ...
  val io = new Bundle {
    val a = slave(Tensor(dataType, Seq(L, C), lanes = inLanes))
    ...
    val stateFill: State = new State with EntryPoint {
      whenIsActive {
        ...
        when(inTensor.stream.valid) {
          tempVec(channelCount.value) := inTensor.stream.payload(0)
          ...
          when(channelCount.willOverflowIfInc) {
            ...
            shiftReg(windowSize - 1) := inTensor.stream.payload(0)
...
object seq2col {
  def apply[T <: Data](a: Tensor[T], kernelSize: Int, outLanes: Int): Tensor[T] = {
    ...
    val comp = Seq2ColOp(a.dataType, a.shape(0), a.shape(1), kernelSize, outLanes, inLanes = a.lanes)
    comp.io.a <> a
    comp.io.c
  }
}
