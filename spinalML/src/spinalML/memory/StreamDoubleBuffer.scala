 ...
 *
  * `io.residentHold` (optional port, weight-residency control plane):
 ...
 *
  * Prefetch surface (with the residency port set): loading into the
...
case class StreamDoubleBuffer[T <: Data](dataType: HardType[T], depth: Int, lanes: Int,
  ...
  val io = new Bundle {
    ...
    // Residency hold — neutralises nextTile when asserted.
    ...
    // Prefetch surface (present when the residency port is).
