...
async def run_ternary_test(dut, op_name, dtype_name, dtype, test_values, is_floatml, expected_bits_fn, true_math_fn, edge_cases=None):
    ...
    for val_x, val_a, val_b in test_values:
        ...
        # Injection
        if is_floatml:
            def assign_float(dut_prefix, val_bits):
                sign = (val_bits >> (dtype.exp_bits + dtype.mant_bits)) & 1
                exp_val = (val_bits >> dtype.mant_bits) & ((1 << dtype.exp_bits) - 1)
                mant = val_bits & ((1 << dtype.mant_bits) - 1)
                getattr(dut, f"{dut_prefix}_sign").value = sign
                getattr(dut, f"{dut_prefix}_exponent").value = exp_val
                getattr(dut, f"{dut_prefix}_mantissa").value = mant
                
            assign_float("io_x_stream_payload_0", val_x_bits)
            assign_float("io_x_stream_payload_1", val_x_bits)
            assign_float("io_a_stream_payload_0", val_a_bits)
            assign_float("io_a_stream_payload_1", val_a_bits)
            assign_float("io_b_stream_payload_0", val_b_bits)
            assign_float("io_b_stream_payload_1", val_b_bits)
        else:
            dut.io_x_stream_payload_0.value = val_x_bits
            dut.io_x_stream_payload_1.value = val_x_bits
            dut.io_a_stream_payload_0.value = val_a_bits
            dut.io_a_stream_payload_1.value = val_a_bits
            dut.io_b_stream_payload_0.value = val_b_bits
            dut.io_b_stream_payload_1.value = val_b_bits
        ...
        assert dut.io_c_stream_valid.value == 1, f"Timeout for ({val_x}, {val_a}, {val_b})!"
