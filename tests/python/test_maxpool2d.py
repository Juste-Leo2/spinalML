...
async def run_maxpool2d_residue_test(dut, dtype_name, dtype, frames, poolSize, stride):
    clock = Clock(dut.clk, 10, units="ns")
    cocotb.start_soon(clock.start())
    dut.reset.value = 1
    await RisingEdge(dut.clk)
    ...
    for fi, frame in enumerate(frames):
        ...
        Y_expected = maxpool2d_hw(frame, poolSize, stride, dtype)
        for i in range(H_out):
            for j in range(W_out):
...
@cocotb.test()
async def cocotb_maxpool2d_residue_i8(dut):
    ...
    await run_maxpool2d_residue_test(dut, "I8", I8, frames, 2, 2)
...
def test_pytest_maxpool2d_residue_i8(request): run_pool_sim("MaxPool2D", "residue", "cocotb_maxpool2d_residue_i8", "MaxPool2DResidueTestComp", request)
