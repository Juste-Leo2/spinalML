...
from golden_models.ops import avgpool2d_hw
...
async def run_avgpool2d_test(dut, op_name, dtype_name, dtype, X, poolSize, stride, is_floatml, collect=None):
    ...
    # True Math (for logging true error): true average with real division
    X_np = np.array(X)
    Y_true = []
    for i in range(H_out):
        ...
        for j in range(W_out):
            ...
            if is_3d:
                row.append(np.mean(window.reshape(poolSize * poolSize, -1), axis=0).tolist())
            else:
    ...
    # Exact HW Math
    Y_expected = avgpool2d_hw(X, poolSize, stride, dtype)
...
@cocotb.test()
async def cocotb_avgpool2d_i8(dut):
    collect = {"out": [], "true": []}
    for _ in range(DEFAULT_NUM_TRIALS):
        X = get_random_tensor((4, 4), 10.0, True)
        await run_avgpool2d_test(dut, "AvgPool2D", "I8", I8, X, 2, 2, False, collect=collect)
    details = f"X=4x4, K=2, stride=2, trials={DEFAULT_NUM_TRIALS}, seed={int(os.environ.get('SPINALML_SEED', SEED))}"
    log_msg = log_true_math_error("AvgPool2D", "I8", I8, False, collect["out"], collect["true"], details=details)
    dut._log.info(log_msg)

@cocotb.test()
async def cocotb_avgpool2d_fp8(dut):
    collect = {"out": [], "true": []}
    for _ in range(DEFAULT_NUM_TRIALS):
        X = get_random_tensor((4, 4), 5.0, False)
        await run_avgpool2d_test(dut, "AvgPool2D", "FP8", FP8_E4M3, X, 2, 2, True, collect=collect)
    details = f"X=4x4, K=2, stride=2, trials={DEFAULT_NUM_TRIALS}, seed={int(os.environ.get('SPINALML_SEED', SEED))}"
    log_msg = log_true_math_error("AvgPool2D", "FP8", FP8_E4M3, True, collect["out"], collect["true"], details=details)
    dut._log.info(log_msg)

@cocotb.test()
async def cocotb_avgpool2d_i16(dut):
    collect = {"out": [], "true": []}
    for _ in range(DEFAULT_NUM_TRIALS):
        X = get_random_tensor((4, 4), 100.0, True)
        await run_avgpool2d_test(dut, "AvgPool2D", "I16", I16, X, 2, 2, False, collect=collect)
    details = f"X=4x4, K=2, stride=2, trials={DEFAULT_NUM_TRIALS}, seed={int(os.environ.get('SPINALML_SEED', SEED))}"
    log_msg = log_true_math_error("AvgPool2D", "I16", I16, False, collect["out"], collect["true"], details=details)
    dut._log.info(log_msg)

@cocotb.test()
async def cocotb_avgpool2d_bf16(dut):
    collect = {"out": [], "true": []}
    for _ in range(DEFAULT_NUM_TRIALS):
        X = get_random_tensor((4, 4), 5.0, False)
        await run_avgpool2d_test(dut, "AvgPool2D", "BF16", BF16, X, 2, 2, True, collect=collect)
    details = f"X=4x4, K=2, stride=2, trials={DEFAULT_NUM_TRIALS}, seed={int(os.environ.get('SPINALML_SEED', SEED))}"
...
async def cocotb_avgpool2dmulti_i8(dut):
    ...
    for _ in range(DEFAULT_NUM_TRIALS):
        ...
        await run_avgpool2d_test(dut, "AvgPool2DMulti", "I8", I8, X, 2, 2, False, collect=collect)
    details = f"X=4x4x2, K=2, stride=2, trials={DEFAULT_NUM_TRIALS}, seed={int(os.environ.get('SPINALML_SEED', SEED))}"
    log_msg = log_true_math_error("AvgPool2DMulti", "I8", I8, False, collect["out"], collect["true"], details=details)
    dut._log.info(log_msg)

@cocotb.test()
async def cocotb_avgpool2dmulti_fp8(dut):
    collect = {"out": [], "true": []}
    for _ in range(DEFAULT_NUM_TRIALS):
        X = get_random_tensor((4, 4, 2), 5.0, False)
        await run_avgpool2d_test(dut, "AvgPool2DMulti", "FP8", FP8_E4M3, X, 2, 2, True, collect=collect)
    details = f"X=4x4x2, K=2, stride=2, trials={DEFAULT_NUM_TRIALS}, seed={int(os.environ.get('SPINALML_SEED', SEED))}"
    log_msg = log_true_math_error("AvgPool2DMulti", "FP8", FP8_E4M3, True, collect["out"], collect["true"], details=details)
    dut._log.info(log_msg)
...
def test_pytest_avgpool2d_i8(request): run_pool_sim("AvgPool2D", "I8", "cocotb_avgpool2d_i8", "AvgPool2DTestComp", request)
def test_pytest_avgpool2d_fp8(request): run_pool_sim("AvgPool2D", "FP8", "cocotb_avgpool2d_fp8", "AvgPool2DTestComp", request)
def test_pytest_avgpool2d_i16(request): run_pool_sim("AvgPool2D", "I16", "cocotb_avgpool2d_i16", "AvgPool2DTestComp", request)
def test_pytest_avgpool2d_bf16(request): run_pool_sim("AvgPool2D", "BF16", "cocotb_avgpool2d_bf16", "AvgPool2DTestComp", request)
